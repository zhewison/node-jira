...
var JiraApi = exports.JiraApi = function(protocol, host, port, username, password, apiVersion, verbose, strictSSL, oauth) {
    ...
    this.doRequest = function(options, callback) {
        if(oauth && oauth.consumer_key && oauth.private_key) {
          options.oauth = {
...
(function() {
    ...
    // *  success string
    //
    // [Jira Doc](https://docs.atlassian.com/jira/REST/latest/#d2e2396)
    this.deleteComment = function(issueId, commentId, callback){
        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/issue/' + issueId + '/comment/' + commentId),
            method: 'DELETE',
            followAllRedirects: true,
            json: true
        };
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            callback(response.statusCode + ': Error while deleting comment');
        });
    };
    ...
    // ### Returns ###
    // *  error: a string with the error
    // *  permissions: array of the permissions for a project
    //
    // [Jira Doc](https://docs.atlassian.com/jira/REST/latest/#d2e5354)
    this.getPermissions = function(project, callback) {

        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/mypermissions?projectKey=' + project),
            method: 'GET'
        };
    ...
    //
    // [Jira Doc](https://docs.atlassian.com/jira/REST/latest/#d2e5354)
    this.getCreateMeta = function(project, callback) {

        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/issue/createmeta?projectKeys=' + project + '&issuetypeNames=Improvement&expand=projects.issuetypes.fields'),
            method: 'GET'
        };
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            if (response.statusCode !== 200) {
                callback(response.statusCode + ': Unable to connect to JIRA during getCreateMeta.');
                return;
            }
